...
### Interface
  - 接口中每一个方法是隐式抽象的, 会被隐式的指定为 `public abstract`
  - 接口中可以含有变量, 会被隐式的指定为 `public static final`
  - 抽象类和接口的区别
    - 抽象类中的方法可以有方法体, 接口中的方法不能被实现
    - 一个类只能继承一个抽象类，但可以实现多个接口
    - 接口中不能含有静态代码块以及静态方法, 抽象类是可以有
  - JDK 1.8 以后, 接口中允许有*默认方法*(default关键字修饰)和*静态方法*, 即函数式接口, 用于Lambda表达式
  - 接口的声明和接口的方法都是隐式抽象的, 不需要`abstract`关键字
  - 类在实现接口的方法时，不能抛出检查性异常，只能在接口中，或者继承接口的抽象类中抛出该检查性异常
  - 接口可以继承多个接口
  - 标记接口: 没有任何方法和属性的接口, 它仅仅表明它的类属于一个特定的类型, 供其他代码来测试允许做一些事情
    - 作用: 可以用 instanceof 进行类型查询, 一些容器如 servlet 容器或运行环境依赖标记接口识别类是否需要进行某种处理, 如 serialialbe 接口标记类需要进行序列化操作
    - 目的:
      - 建立一个公共的父接口
      - 向一个类添加数据类型

### Encapsulation
  - 封装是指一种将抽象性函式接口的实现细节部分包装、隐藏起来的方法。最主要的功能在于我们能修改自己的实现代码，而不用修改那些调用的代码程序
  - 优点:
    - 良好的封装能够减少耦合
    - 类内部的结构可以自由修改
    - 可以对成员变量进行更精确的控制
  ...
  - 实现Java封装的步骤
    ...
    - 对私有属性提供对外的公共方法访问，也就是创建一对 getter setter 方法
